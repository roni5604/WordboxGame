...
class _RaceGameScreenState extends ConsumerState<RaceGameScreen> {
  ...
  Future<void> _initRace() async {
    ...
    final bots = widget.config.bots
        .map(
          (b) =>
              BotPlayer(name: b.name, difficulty: b.difficulty, board: board),
        )
        .toList();
    ...
    _ticker = Timer.periodic(tickDuration, (timer) {
      ...
      setState(() {
        ...
        for (final bot in _bots) {
          ...
          if (found != null) {
            _showBanner(
              '${bot.name} מצא/ה מילה! (${HebrewTrie.toDisplayWord(found.normalizedWord)})',
            );
          }
        }
  ...
  List<ScoreboardEntry> _buildScoreboard() {
    final session = _session;
    if (session == null) return [];
    // תמונת/שם הפרופיל האמיתי של השחקן/ית - מוצגים במקום קמע גנרי (ראו
    // live_scoreboard.dart), כדי שגם בדו-קרב מול המחשב יהיה ברור "מי אני".
    ...
    final entries = <ScoreboardEntry>[
      ScoreboardEntry(
        ...
        score: session.score,
        isHuman: true,
        avatarId: profile?.avatarId ?? 'detective',
        photoUrl: authUser?.photoUrl,
      ),
      for (final bot in _bots)
        ScoreboardEntry(name: bot.name, score: bot.score, isHuman: false),
                      ...
                      Padding(
                        ...
                        child: TimerBar(
                          progress:
                              ...
                              widget.config.timeLimit.inMilliseconds,
                          urgent:
                              _remaining.inSeconds <
                              (widget.config.timeLimit.inSeconds * 0.2),
                        ),
                      ),
